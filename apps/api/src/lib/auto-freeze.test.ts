 ...
 *
 * Uses vi.useFakeTimers() to simulate 30s grace + 5min freeze timers (D-05, Phase 14 —
 * lowered from 15min) without waiting real time.
 */
...
describe('auto-freeze tracker', () => {
  ...
  it('Test 1: presence drop -> after 30s grace + 5min freeze fires', async () => {
    ...
    await Promise.resolve()
    // Still in 5-min countdown, no freeze yet
    expect(mockUpdate).not.toHaveBeenCalled()

    // Advance past the 5-min countdown (D-05)
  ...
  it('Test 3: presence drop -> 60s absence -> reconnect at 2-min mark -> NO freeze fires', async () => {
    const sessionId = 'session-3'
    const creatorId = 'creator-3'

    onCreatorPresenceChange(sessionId, creatorId, false, mockSupabase as never)

    // Advance past 30s grace — real absence starts (5min countdown begins, D-05)
    vi.advanceTimersByTime(31_000)
    await Promise.resolve()

    // Reconnect at the 2-min mark of the 5-min countdown (safely before the freeze fires)
  ...
  it('Test 4: two presence drops in quick succession do NOT stack timers — only one fires', async () => {
    ...
    // Advance past total time needed for both potential freezes (D-05: 5-min freeze window)
