...
- [x] **AI-01**: Creator provides their Anthropic API key via a clearly labeled UI flow ("Connect Access Key") — key is stored encrypted in Supabase user metadata, never transmitted to the browser
- [x] **AI-02**: All Claude API calls are made exclusively server-side (Hono backend) — the API key is never present in any client-side request
- [x] **AI-03**: AI responses stream token-by-token into the chat stream with visible streaming indicator
- [x] **AI-04**: AI output uses strict dual-channel streaming: `text_stream` routes to chat bubbles; `ui_mutation_block` is a separate JSON tool call routed to the analytics panel
- [x] **AI-05**: Every `ui_mutation_block` is validated against a Zod schema before touching React state; corrupted or schema-invalid payloads are silently discarded and the last stable panel state is preserved
...
- [x] **AI-08**: Server-side sliding window context strategy: pass only the last 8 raw messages of the active branch to Claude; prepend a background-compressed summary of the older conversation history (generated by a lighter/faster model call) to preserve context without unbounded latency growth
...
- [x] **AI-10**: Upfront Key Verification — when the creator submits their API key (`AI-01`), the Hono backend immediately executes a minimal handshake request against the Anthropic API before persisting the key; if the API returns an authentication error, the save is aborted and the frontend displays an immediate, specific error message; keys that pass verification are saved encrypted
