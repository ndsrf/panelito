...
- **D-10** `[informational]`**:** Dynamic, session-specific objective elicitation (agents proactively asking the group to state their goal — upfront for coaching-type Blueprints, reactively after N unclear messages for debate-type Blueprints, not needed for others) is **out of scope for Phase 12** and captured as a Phase 13 candidate (see `<deferred>`). Boundary-setting decision only — no plan implements it; tracked for scope, not for coverage.
